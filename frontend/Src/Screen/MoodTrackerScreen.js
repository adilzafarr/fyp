...
const MoodTrackerScreen = () => {
  ...
  const moods = [
    ...
    { name: 'Frustrated', icon: 'warning', color: '#FF9800' },
    { name: 'Dissatisfied', icon: 'close-circle', color: '#9C27B0' },
  ];

  useEffect(() => {
    loadData();
    // Only add a random mood for today if there's no entry for today in the data
  ...
  const loadData = async () => {
    try {
      ...
      setMoodHistory(dataPath);
      calculateTopEmotion(dataPath);
      updateCurrentMonthEmotions(dataPath);
    } finally {
      setIsLoading(false);
    }
  };
