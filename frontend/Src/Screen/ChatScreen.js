...
const ChatScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    // Make sure tab bar is visible initially
    navigation.setOptions({ tabBarStyle: { display: 'flex' } });
  ...
  useEffect(() => {
    const initializeChat = async () => {
      try {
        // Check for chatId in route params (from stack navigator)
        // or in navigation state (from tab navigator)
        const chatId = route.params?.chatId || navigation.getState().routes.find(r => r.name === 'ChatTab')?.params?.chatId;
  ...
  }, [route.params?.chatId, navigation.getState().routes.find(r => r.name === 'ChatTab')?.params?.chatId]);

  // Add a new useEffect to handle navigation state changes
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', () => {
      const newChatId = route.params?.chatId || navigation.getState().routes.find(r => r.name === 'ChatTab')?.params?.chatId;
      console.log('Screen focused, chatId:', newChatId);
    ...
    return unsubscribe;
  }, [navigation, route.params?.chatId, chatId]);
